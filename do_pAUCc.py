...
def partial_c_statistic(posScores, negScores, posWeights, negWeights, yrange, xrange):
    ...
    if count_area == 0:
        ...
        C_i = (1/2)*(count_x/count_area) + (1/2)*(count_y/count_area)
        Cy = count_y / count_area
        Cx = count_x / count_area
    #endif

    count_areax = N * partP
    count_areay = P * partN

    if   count_areax == 0 and count_areay == 0:
        Cn_i = 0
    else:
        Cn_i = (C_i * (2*count_area)) / (count_areax + count_areay)
        #Cn_i = (C_i * 2) / (count_areax + count_areay)
    #endif

    if count_areax == 0:
        AvgSpec = 0
    else:
        AvgSpec = count_x/count_areax
    #endif

    if count_areay == 0:
        AvgSens = 0
    else:
        AvgSens = count_y/count_areay
    #endif
    ...
    #return C_i, Cn_i, cLocal_i, count_x/count_area, count_y/count_area, AvgSpec, AvgSens, count_x, count_y, \
    #       count_area, count_areax, count_areay

    #       C_i, Cn_i, cLocal_i, count_x1, count_y1, count_x2, count_y2, count_x, count_y, \
    #       whole_area, count_areax, count_areay \
    #            = partial_c_statistic(posScores, negScores, posWeights, negWeights, posIndexC, negIndexC)
    group_measures_dict = {'C_i':         C_i,            # cDelta
                           'Cn_i':        Cn_i,           # cDeltan
                           'CLocal_i':    cLocal_i,       # cLocal
                           'Cy':          Cy,             # area:     cn1 = cp/whole_area
                           'Cx':          Cx,             # area:     cp1 = cn/whole_area
                           'AvgSens':     AvgSens,        # distance: cn2 = cn_normalized
                           'AvgSpec':     AvgSpec,        # distance: cp2 = cp_normalized
                           'count_y':     count_y,        # count:    cn  (U statistic y part)
                           'count_x':     count_x,        # count:    cp  (U statistic x part)
                           'Uy':          count_y,        # count:    cn  (U statistic y part)
                           'Ux':          count_x,        # count:    cp  (U statistic x part)
                           'count_area':  count_area,     # count:    whole_area
                           'count_areax': count_areax,    # count:    horizontal_stripe_area
                           'count_areay': count_areay     # count:    vertical_stripe_area
                          }
...
def makeLabels01(labels, posclass):
    ...
    newlabels = list(labels).copy()  # here i must specify a list, or else the indexing below creates net new entries for a longer array
    for i in posIndex:
        newlabels[i] = 1  # this innocent looking code is wrong for pandas.Series, but fine for list
    for i in negIndex:
        newlabels[i] = 0
    return newlabels
#enddef
...
def do_pAUCc(mode='',          index=1,         pAUCrange=[],
    ...
    gm = partial_c_statistic(posScores, negScores, posWeights, negWeights, posIndexC, negIndexC)
    if not quiet:
        print(f"{'Cy (=pAUC)':12s} = {gm['Cy']:0.4f}    from {gm['count_y']:0.2f}/{gm['count_area']:0.2f}")
        print(f"{'Cx (=pAUCx)':12s} = {gm['Cx']:0.4f}    from {gm['count_x']:0.2f}/{gm['count_area']:0.2f}")
        print(f"{'C_i':12s} = {gm['C_i']:0.4f}")
        print(f"{'AvgSens (~pAUCn)':12s} = {gm['AvgSens']:0.4f}    from {gm['count_y']:0.2f}/{gm['count_areay']:0.2f}")
        print(f"{'AvgSpec (~pAUCxn)':12s}= {gm['AvgSpec']:0.4f}    from {gm['count_x']:0.2f}/{gm['count_areax']:0.2f}")
        print(f"{'Cn_i':12s} = {gm['Cn_i']:0.4f}")
        print(f"{'cLocal':12s} = {gm['CLocal_i']:0.4f}")
        print(' ')
    #endif
    ...
    C_i   = gm['C_i']
    Cn_i  = gm['Cn_i']
    pass1 = areEpsilonEqual(C_i,                      cpAUC,  'cDelta',    'cpAUC',  ep, quiet)
    #pass2 = areEpsilonEqual(extras_dict['bAvgA'],    cpAUCn, 'bAvgA',     'cpAUCn', ep, quiet)
    pass2 = areEpsilonEqual(Cn_i,                     cpAUCn, 'cDeltan',   'cpAUCn', ep, quiet)
    ...
    return iterationPassed, C_i, cpAUC, pAUC, pAUCx, Cn_i, cpAUCn, pAUCn, pAUCxn, extras_dict
#enddef
